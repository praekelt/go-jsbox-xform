...
var XFormState = State.extend(function (self, name, opts) {
    ...
    :param string_or_function_or_promise opts.xform:
        ...
        a string containing the xform, or a promise that returns with the
        xform. Required.
    ...
    */
    ...
    self.set_next_question = function(question) {
        ...
        if(!question) {
            return self.get_responses()
                .then(self.store_responses)
                .then(self.post_responses)
    ...
    self.post_responses = function(responses) {
        if(!self.results_url) {
            return Q(responses);
        }
        var http = new HttpApi(self.im);
        return http.post(self.results_url, {
                data: responses})
            .then(function() {
                return responses;
            }, function(err) {
                return self.im.log.error([
                    ...
                    err.message])
                    .then(function() {
                        self.metadata.display_text = self.result_error_message;
                        self.metadata.xforms_state = null;
                        return false;
                    });
            });
    ...
    self.store_responses = function(responses) {
        if(self.contact_namespace) {
            self.contact.extra[self.contact_namespace] = responses;
